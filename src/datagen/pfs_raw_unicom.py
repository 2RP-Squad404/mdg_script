...
def datagen_funcionarios(num_records):
    ...
    for _ in range(num_records):
        criar_funcionarios = {
            ...
            'flg_situacao': faker.random_element(elements=['Ativo', 'Inativo']),
            'num_cpf': faker.cpf(),
            'num_rg': faker.bothify(text='??########'),
            'dat_admissao': faker.date_between(start_date='-20y', end_date='today').strftime('%Y-%m-%d %H:%M:%S'),
            'dat_demissao': faker.date_between(start_date='today', end_date='+1y').strftime('%Y-%m-%d %H:%M:%S'),
            'dat_inic_afastamento': faker.date_time().strftime('%Y-%m-%d %H:%M:%S'),
            'dat_term_afastamento': faker.date_time().strftime('%Y-%m-%d %H:%M:%S'),
